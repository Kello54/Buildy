...
// ---------- Dark Mode ----------
darkToggle.addEventListener('click',()=>{document.body.classList.toggle('dark'); darkToggle.textContent=document.body.classList.contains('dark')?'☀️':'🌙'; renderMainChart();});
...
function refreshAll(){
    save(); renderFilterOptions(); renderTable(); renderMainChart();
}
...
function renderTable(){
    ...
    materials.forEach(m=>{
        ...
        m.prices.forEach(p=>{
            ...
            const total=(p.price*p.quantity)+p.delivery+(p.price*p.quantity*p.vat/100);
            // Determine cheapest / expensive / mid
