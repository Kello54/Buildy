...
function refreshAll(){
    ...
    renderTable(); 
    renderMainChartWithTrends(); 
    renderTrendAlerts(); // Trend badges animate
}
...
function renderTable(){
    ...
    const filterSup=filterSupplier.value.toLowerCase();
    materials.forEach(m=>{
        if(filterMat && !m.name.toLowerCase().includes(filterMat)) return;
        m.prices.forEach(p=>{
            if(filterSup && p.supplier.toLowerCase()!==filterSup) return;
            const tr=document.createElement('tr');
            const total=(p.price*p.quantity)+p.delivery+(p.price*p.quantity*p.vat/100);
            const totals=m.prices.map(pp=>(pp.price*pp.quantity)+pp.delivery+(pp.price*pp.quantity*pp.vat/100));
            const minTotal=Math.min(...totals); const maxTotal=Math.max(...totals);
            ...
            const canvas=document.createElement('canvas'); canvas.className='canvas-mini'; tdChart.appendChild(canvas); tr.appendChild(tdChart);
...
function renderMiniChart(material, supplier, canvas){
    ...
    new Chart(canvas,{type:'line',data:{datasets:[{label:supplier,data:data,borderColor:'#4b0082',backgroundColor:'rgba(75,0,130,0.2)',fill:true,tension:0.3}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{display:false},y:{display:false}}}});
}

// ------------------ Main Chart with Trend Overlay ------------------
function renderMainChartWithTrends(){
    if(chart) chart.destroy();
    const labels=materials.map(m=>m.name);
    const datasets=[]; const colors=['#4b0082','#6a0dad','#a67bf0','#8d5bd6','#c299ff']; let colorIndex=0;
    const filterSup=filterSupplier.value.toLowerCase();
    materials.forEach(m=>{
        m.prices.forEach(p=>{
            if(filterSup && p.supplier.toLowerCase()!==filterSup) return;
            const historyTotals=m.history.filter(h=>h.supplier===p.supplier).map(h=>(h.price*h.quantity)+h.delivery+(h.price*h.quantity*h.vat/100));
            datasets.push({
                label:`${p.supplier} (${m.name})`,
                data: historyTotals.length ? historyTotals : [0],
                type:'line',
                borderColor:colors[colorIndex%colors.length],
                backgroundColor:colors[colorIndex%colors.length]+'66',
                fill:false,
                tension:0.3
            });
            colorIndex++;
    ...
    chart=new Chart(ctx,{type:'bar',data:{labels:labels,datasets:datasets},options:{responsive:true,plugins:{legend:{position:'bottom'}},scales:{y:{beginAtZero:true}}}});
}

// ------------------ Trend Alerts ------------------
function renderTrendAlerts() {
    const container = $('trendResults');
    container.innerHTML='';
    const filterSup = filterSupplier.value.toLowerCase();
    materials.forEach(mat => {
        mat.prices.forEach(p => {
            if(filterSup && p.supplier.toLowerCase() !== filterSup) return;
            const history = mat.history.filter(h => h.supplier === p.supplier);
            if(history.length < 2) return;
            const lastTotal = (history[history.length-1].price*history[history.length-1].quantity)+history[history.length-1].delivery+((history[history.length-1].price*history[history.length-1].quantity*history[history.length-1].vat)/100);
            const prevTotal = (history[history.length-2].price*history[history.length-2].quantity)+history[history.length-2].delivery+((history[history.length-2].price*history[history.length-2].quantity*history[history.length-2].vat)/100);
            let trendClass='trend-stable', symbol='→';
            if(lastTotal < prevTotal){ trendClass='trend-down'; symbol='↓'; }
            else if(lastTotal > prevTotal){ trendClass='trend-up'; symbol='↑'; }
            const badge = document.createElement('span');
            badge.className = 'trend-badge '+trendClass;
            badge.textContent = `${p.supplier} (${mat.name}) ${symbol}`;
